					...
					<!--<div class="offcanvas offcanvas-end border-0" tabindex="-1" id="theme-settings-offcanvas">
						...
						<div class="offcanvas-footer border-top p-3 text-center">
							<div class="row">
								...
								<div class="col-6">
								<button type="button" class="btn btn-primary w-100" id="customizerclose-btn" data-bs-dismiss="offcanvas" aria-label="Close">Close</button>
								</div>
							</div>
						</div>
					</div>-->
					<!-- importnant for delete modal-->
					...
					<!-- custom js -->
					<script src="/assets/custom/jquery.form.min.js?up=<?= $this->version_update ?>"></script>
					<script src="/assets/custom/submit.js?up=<?= $this->version_update ?>"></script>
					<script src="/assets/custom/custom.js?up=<?= $this->version_update ?>"></script>
					<script src="/assets/custom/toastify.js?up=<?= $this->version_update ?>"></script>
					...
					<script>
						$(document).ready(function() {
							...
							// $.ajax({
								...
								// success: function(res) {
									// var notificationCount = JSON.parse(res);
								...
								// $.ajax({
									...
									// success: function(res) {
										...
										// data.notifications.forEach(n => {
											// list += '<div class="text-reset notification-item d-block dropdown-item position-relative">';
											// list += '<div class="d-flex">';
											// list += '<div class="avatar-xs me-3 flex-shrink-0">';
											// list += '<span class="avatar-title bg-info-subtle text-info rounded-circle fs-16">';
											// list += '<i class="bx bx-badge-check"></i>';
											// list += '</span>';
											// list += '</div>';
											// list += '<div class="flex-grow-1">';

											// list += '<a href="/Notification" class="stretched-link" onclick="redirectToNotification(' + n.id + '); return false;">';
											// list += '<h6 class="mt-0 mb-2 lh-base">' + n.title + '</h6>';
											// list += '</a>';

											// list += '<p class="mb-0 fs-11 fw-medium text-uppercase text-muted">';
											// list += '<span><i class="mdi mdi-clock-outline"></i> ' + timeAgo(n.c_date) + '</span>';
											// list += '</p>';
											// list += '</div>';
											// list += '</div>';
											// list += '</div>';
										...
										// list += '<div class="my-3 text-center view-all">';
										// list += '<button type="button" class="btn btn-soft-success waves-effect waves-light">';
										// list += 'View All Notifications (' + data.countMoreThanFive + ') <i class="ri-arrow-right-line align-middle"></i></button>';
										// list += '</div>';
										...
										// if (notificationList) {
											// SimpleBar.instances.forEach((instance) => instance.unMount());
											// new SimpleBar(notificationList);
										// }
						...
						function init_dataTable(selector) {
							var tmpdt = $(selector).DataTable({
								pageLength: 100,
								dom: "Blfrtip",
								buttons: ["copy", "csv", "excel", "print", "pdf"]
							});
							DataTablesForAjax.push(tmpdt);
						}

						function init_dataTable_class(selector) {
						...
						function destroy_datatable() {
							if (Array.isArray(DataTablesForAjax)) {
								DataTablesForAjax.forEach(function(dataTable) {
									dataTable.destroy(); // Destroy each DataTable instance
								});
								DataTablesForAjax = []; // Clear the array
							}
						}
					</script>
					</body>
